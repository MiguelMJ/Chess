...
class Board: public sf::Drawable, public sf::Transformable{
    ...
    public:
        ...
        void mark(unsigned r, unsigned c, sf::Color col, float th=1){
            if(r < R && c < C){
                sf::RectangleShape mark(sf::Vector2f(size-th*2,size-th*2));
                mark.setFillColor(sf::Color(0,0,0,0));
                mark.setOutlineColor(col);
                mark.setOutlineThickness(th);
                mark.setPosition(getPosition() + sf::Vector2f(c*size+th,r*size+th));
        ...
        sf::Vector2i getCell(float x, float y){
            getCell(sf::Vector2f(x, y));
        }
